...
function have_read_access($listId = null)
{
    ...
    $field = (object)$db->dq("SELECT delta FROM {mytinytodo_fields} WHERE id = ?", $field_id)->fetch_assoc();

    $field_info = field_info_field_by_id($field->delta);
    return user_access('view ' . $field_info['field_name'], $user) === true;
}
...
function have_write_access($listId = null)
{
        ...
        $field = (object)$db->dq("SELECT entity_id, entity_type FROM {mytinytodo_fields} WHERE id = ?", $field_id)->fetch_assoc();

		if ($field->entity_type == 'node') {
			$node = (object)$db->dq("SELECT nid, uid, type FROM {node} WHERE nid = ?", $field->entity_id)->fetch_assoc();

			if (!$node)
					return false;
			else if ($node->uid != $user->uid) {
					 if (!user_access('edit any ' . $node->type . ' content', $user))
							jsonExit( array('total'=>0, 'list'=>array(), 'denied'=>1) );
					else
							return true;
			} else {
					 if (!user_access('edit own ' . $node->type . ' content', $user))
							jsonExit( array('total'=>0, 'list'=>array(), 'denied'=>1) );
					else
							return true;
			}
		} else if ($field->entity_type == 'user') {
			return user_access('access user profiles', $user);
		}
}
