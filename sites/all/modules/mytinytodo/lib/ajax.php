...
elseif(isset($_GET['editTask']))
{
	...
	$db->dq("UPDATE {mytinytodo_todos} SET title=?,note=?,prio=?,tags=?,tags_ids=?,duedate=?,d_edited=? WHERE id=$id",
...
elseif(isset($_GET['renameList']))
{
	...
	$r = $db->sqa("SELECT * FROM {mytinytodo_lists} WHERE id=$id AND field_id = ?", array($field_id));
	$t['list'][] = prepareList($r);
	jsonExit($t);
}
